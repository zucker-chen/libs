...
int main (void)
{
    ...
    ret = test_diskio(0, 3, buff, sizeof buff);
    if (ret) {
    ...
    if (ret != 0) {
        printf("%s(%d) f_fdisk ERROR ret = %d\n", __FUNCTION__, __LINE__, ret);
    }
    
    ret = f_mkfs("", FM_FAT32, 0, NULL, 0);
    if (ret != 0) {
        printf("%s(%d) f_mkfs ERROR ret = %d\n", __FUNCTION__, __LINE__, ret);
    }

	ret = f_mount(&FatFs, "", 1);		/* Give a work area to the default drive */
    if (ret != 0) {
        printf("%s(%d) f_mount ERROR ret = %d\n", __FUNCTION__, __LINE__, ret);
    }
    //printf("%s(%d) %d %d %d %d %d %d\n", __FUNCTION__, __LINE__, FatFs.n_fatent, FatFs.fsize, FatFs.volbase, FatFs.fatbase, FatFs.dirbase, FatFs.csize);

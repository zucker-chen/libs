...
import sys
import json
from cozepy import COZE_CN_BASE_URL, Coze, JWTAuth, JWTOAuthApp, Stream, WorkflowEvent, WorkflowEventType  # noqa
import requests
import re

class CozeWorkflowHandler:
    def __init__(self, oauth_app, base_url, workflow_id, webhook_url):
        self.coze = Coze(auth=JWTAuth(oauth_app=oauth_app), base_url=base_url)
...
if __name__ == "__main__":
    ...
    client_id = "1192049588811"  # 应用ID
    public_key_id = "oQr3Fri8U6OAaEhpVJeXF3R6zX1xcZUb7gz0n6wxJ9Y"  # 公钥ID
    private_key_path = os.path.join(base_path, 'private_key.pem')  # 私钥路径
    
    try:
        with open(private_key_path, "r") as f:
            private_key = f.read()
    except FileNotFoundError:
        print(f"未找到私钥文件: {private_key_path}")
        sys.exit(1)
    ...
    handler = CozeWorkflowHandler(jwt_oauth_app, coze_api_base, workflow_id, webhook_url)

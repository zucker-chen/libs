...
target_ver="Release_3.10"
output_path="$(cd `dirname $0`; pwd)/ELFIO-${target_ver}/build"
...
if [ ! -f ${target_ver}.tar.gz ]; then
	wget https://github.com/serge1/ELFIO/archive/refs/tags/${target_ver}.tar.gz -O ${target_ver}.tar.gz
...
# ./configure
pri_cflags="$cross_pri_cflags CXXFLAGS=-std=c++11 --prefix=$output_path"

...
ringbuf_t *rb[2];
rtmps_stream_info_t stream_info[2];
...
static void * get_stream_thdcb(void * arg)
{
	...
	while (thd_running[nCh] == 1) 
	{
		...
		stPkg.stream_type = nCh == 0 ? 12 : 7; //RTP_PAYLOAD_H265;
		stPkg.key_frame = 0;
		stPkg.pts = stHiVencStream.pstPack[0].u64PTS / 1000 * 90;	// us -> 1/90000
		nRet = ringbuf_write_get_unit(rb[nCh], (unsigned char **)&p, sizeof(rtmps_frame_info_t) + stPkg.data_len);
		if (nRet != 0) {
			printf("func = %s, line = %d:  ch = %d, nFrameSize = %d\n", __FUNCTION__, __LINE__, nCh, nFrameSize);
			continue; 
		}
		memcpy(p, &stPkg, sizeof(rtmps_frame_info_t));
		p += sizeof(rtmps_frame_info_t);
		...
		ringbuf_write_put_unit(rb[nCh], sizeof(rtmps_frame_info_t) + stPkg.data_len);
...
static int get_stream_info(char *channel_name, rtmps_stream_info_t *strem_info)
{
...
static int get_rb_stream(void *rb_reader, rtmps_frame_info_t **pkg)
{
	if (rb_reader == NULL) {
		return -1;
	}

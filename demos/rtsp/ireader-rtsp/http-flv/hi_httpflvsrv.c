...
static void * get_stream_thdcb(void * arg)
{
	...
	while (thd_running[nCh] == 1) 
	{
		...
		nRet = ringbuf_write_get_unit(rb[nCh], (unsigned char **)&p, sizeof(httpflv_frame_info_t) + stPkg.data_len);
		if (nRet != 0) {
			printf("func = %s, line = %d:  ch = %d, nFrameSize = %d\n", __FUNCTION__, __LINE__, nCh, nFrameSize);
			continue; 
		}
		memcpy(p, &stPkg, sizeof(httpflv_frame_info_t));
		p += sizeof(httpflv_frame_info_t);
		...
		ringbuf_write_put_unit(rb[nCh], sizeof(httpflv_frame_info_t) + stPkg.data_len);
...
static int get_stream_info(char *channel_name, httpflv_stream_info_t *strem_info)
{

...
if [ ! -f ${target_ver}.tar.gz ]; then
	wget http://git.savannah.gnu.org/cgit/freetype/freetype2.git/snapshot/freetype2-VER-2-8.tar.gz -O ${target_ver}.tar.gz
...
# ./configure
pri_cflags="$cross_pri_cflags --prefix=$output_path --with-zlib=no --with-quickdraw-toolbox=no --with-png=no"
...
# Tips:
# if `sh autogen.sh` fail need install autoconf and libtool first

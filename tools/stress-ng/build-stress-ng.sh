...
if [ $enable_cross_compile = true ]; then
	...
	export CC=${cross_prefix}gcc CPP=${cross_prefix}cpp CXX=${cross_prefix}g++
fi
cd $(dirname "$0")
# Fetch Sources
if [ ! -f ${target_ver}.tar.gz ]; then
    wget https://kernel.ubuntu.com/~cking/tarballs/stress-ng/${target_ver}.tar.xz -O ${target_ver}.tar.gz
...
# make & install
export DESTDIR=${output_path}
make -j4 && make install



# Tips:
# v1.14: "stdio.h:1010:1: error: ‘gets’ undeclared here (not in a function)"  =>> commit it or v1.15.
